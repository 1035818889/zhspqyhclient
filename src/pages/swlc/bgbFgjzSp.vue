    ...
    <div>
        ...
        >
            <template slot="content">
                <div v-if="activeIndex === 0">
                    <van-cell-group title="变更原因">
                        <van-field
                                v-model="bgb[0].fqrCname"
                                label="发起人："
                                autosize
                                readonly
                        />
                    ...
                    <van-cell-group title="来文处理意见回复正文">
                        <van-list
                                v-model="loadingBgbLwclyjZws"
                                :finished="finishedBgbLwclyjZws"
                                @load="onLoadBgbLwclyjZws"
                        >
                            <van-cell
                                    v-for="bgbLwclyjZw in bgbLwclyjZws"
                                    :key="bgbLwclyjZw.url"
                                    :title = "bgbLwclyjZw.title"
                                    is-link
                                    center
                                    @click="showFile(bgbLwclyjZw.url,'lwclyjZw')"
                            />
                        </van-list>
                    </van-cell-group>
                    <van-cell-group title="来文处理意见回复附件">
                        <van-list
                                v-model="loadingBgbLwclyjFjs"
                                :finished="finishedBgbLwclyjFjs"
                                @load="onLoadBgbLwclyjFjs"
                        >
                            <van-cell
                                    v-for="bgbLwclyjFj in bgbLwclyjFjs"
                                    :key="bgbLwclyjFj.url"
                                    :title = "bgbLwclyjFj.title"
                                    is-link
                                    center
                                    @click="showFile(bgbLwclyjFj.url,'lwclyjFj')"
                            />
                        </van-list>
                    </van-cell-group>
                </div>
        ...
        <van-goods-action safe-area-inset-bottom style="width: 100%;/*margin-bottom: 55px;*/">
            <van-row style="width: 100%;">
                ...
                <van-col span="8">
                    <van-goods-action-button style="" type="info" text="提交盖章" @click="onClickSendGz" />
                </van-col>
                <van-col span="8">
                    <van-goods-action-button style="border-top-right-radius: 10px;border-bottom-right-radius: 10px;" type="info" text="提交局长" @click="onClickSendJz" />
                </van-col>
            </van-row>
        </van-goods-action>
        <van-popup
    ...
    export default {
        ...
        data() {
            return {
                ...
                popupShow:false,
                activeIndex: 0,
                items: [{ text: '综合意见变更情况' },{ text: '基本信息' }, { text: '正文附件' }, { text: '科室意见' }, { text: '综合意见' }, { text: '审批历史' }],
                jbxx: [],
                loadingJbxx: false,
                finishedJbxx: false,
                ...
                bgbLwclyjZws: [],
                loadingBgbLwclyjZws:false,
                finishedBgbLwclyjZws: false,
                bgbLwclyjFjs: [],
                loadingBgbLwclyjFjs:false,
                finishedBgbLwclyjFjs: false,
                wjysZws: [],
                ...
                apiUrlBgb: this.GLOBAL.serverSrc+'/zhspSwlc/getBgbById.do',
                bgb:[{fqrCname:'',xgyy:'',wjysId:''}],
        ...
        methods: {
            ...
            onLoadBgbLwclyjZws() {
                ...
                this.$http.get(this.apiUrlBgb,{params: {businessId:businessId}}).then(function(response) {
                    this.bgb =response.data;
                },function() {
                ...
                this.$http.get(this.apiUrlLwclyjZws,{params: {businessId:businessId,type:'bgbZhyjb'}}).then(function(response) {
                    this.bgbLwclyjZws=response.data;
                    this.loadingBgbLwclyjZws = false;
                    this.finishedBgbLwclyjZws = true;
                },function() {
                    // eslint-disable-next-line no-console
                    console.log("出错了");
                }).catch(function(response) {
            ...
            onLoadBgbLwclyjFjs() {
                let businessId = this.$route.query.businessId;
                this.$http.get(this.apiUrlLwclyjZws,{params: {businessId:businessId,type:'bgbLwclyjFj'}}).then(function(response) {
                    this.bgbLwclyjFjs=response.data;
                    this.loadingBgbLwclyjFjs = false;
                    this.finishedBgbLwclyjFjs = true;
                },function() {
                    // eslint-disable-next-line no-console
                    console.log("出错了");
                }).catch(function(response) {
            ...
            onClickSendJz(){
                this.submitType = "passToJz";
                this.popupShow = true;
            },
            onClickSendGz(){
                this.submitType = "passToGz";
                this.popupShow = true;
            },
            onClickBack(){
                ...
                this.$dialog.confirm({
                    ...
                    this.$http.get(this.apiUrlOperate,{
                        params: {businessId:businessId,taskId:taskId,
                            opinion:this.value,submitType:this.submitType,opFunction:"swlcBgbFgjzSp"}
            ...
            onLoadJbxx() {
                //let businessId = this.$route.query.businessId;
                let businessId = this.bgb[0].wjysId;
