...
<script>
    ...
    export default {
        ...
        data() {
            return {
                activeIndex: 0,
                items: [{ text: '项目基本信息' }, { text: '综合意见' }, { text: '实施情况说明' }, { text: '建设单位上传的附件' }],
                jbxx: [],
                loadingJbxx: false,
                finishedJbxx: false,
        ...
        methods: {
                ...
                this.$dialog.confirm({
                    ...
                    this.$http.get(this.apiUrlOperate,{
                        params: {businessId:businessId,taskId:taskId,
                            opinion:this.value,submitType:"back",opFunction:"jzOperate"}
            ...
            onClickPass(){
                ...
                this.$dialog.confirm({
                    ...
                    this.$http.get(this.apiUrlOperate,{
                        params: {businessId:businessId,taskId:taskId,
                            opinion:this.value,submitType:"pass",opFunction:"jzOperate"}
                    }).then(function(response) {
                        if("SUCCESS"==response.bodyText){
                            this.$toast.success('提交成功！');
                            window.top.location.href="/";
            ...
            onLoadSsqks() {
                let businessId = this.$route.query.businessId;
                this.$http.get(this.apiUrlFjs,{params: {businessId:businessId,type:'czjzhyjssqk'}}).then(function(response) {
            ...
            onLoadHiss() {
                let businessId = this.$route.query.businessId;
                this.$http.get(this.apiUrlHiss,{params: {businessId:businessId,type:'jz'}}).then(function(response) {
